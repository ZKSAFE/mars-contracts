...
contract QuoterV3 is IQuoter, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    function quoteExactInputSingle(
        ...
        address poolAddr = IUniswapV3Factory(factory).getPool(tokenIn, tokenOut, fee);
        if (poolAddr == address(0) || IUniswapV3Pool(poolAddr).liquidity() == 0) {
            return 0;
        } else {
            // console.log(poolAddr, tokenIn, tokenOut, fee);
        }
    ...
    function aggregate(bytes[] memory callDatas, uint eachGaslimit) public returns (uint[] memory returnDatas) {
        ...
        for (uint i = 0; i < callDatas.length; i++) {
            uint before = gasleft();
            (bool success, bytes memory ret) = address(this).call{gas: eachGaslimit}(callDatas[i]);
            if (success) {
                console.log("[aggregate] success", uint(bytesToBytes32(ret)), "gas used", before - gasleft());
                returnDatas[i] = uint(bytesToBytes32(ret));
            } else {
